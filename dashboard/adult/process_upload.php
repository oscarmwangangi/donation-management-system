            ...
            <div class='modal-dialog' role='document'>
                <div class='modal-content'>
                    <div class='modal-header'>
                        ...
                        <button type='button' class='close' onclick=\"window.location.href='" . ($redirect ? 'view_adult.php?id=' . $_POST['adult_id'] : 'javascript:history.back()') . "';\" aria-label='Close'>
                    ...
                    <div class='modal-footer'>
                        <button type='button' class='btn btn-primary' onclick=\"window.location.href='" . ($redirect ? 'view_adult.php?id=' . $_POST['adult_id'] : 'javascript:history.back()') . "';\">OK</button>
                    </div>
                </div>
            </div>
        </div>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['adult_id'])) {
    ...
    //     $uploadOk = 1;
    // } else {
    //     display_modal('Error', 'The file is not a valid image.');
    //     $uploadOk = 0;
    // }
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["profile_picture"]["tmp_name"], $target_file)) {
            ...
            $sql = "UPDATE adult SET profile_picture = '$target_file' WHERE id = $adult_id";
